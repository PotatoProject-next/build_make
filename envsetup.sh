...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the PotatoDevices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/python/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/python/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
